...
import sqlTutorialData from './sqlTutorialData.json';

const SQLTopicDetail = ({ route, navigation }: any) => {
  const { topic } = route.params;
  const topicData = sqlTutorialData[topic as keyof typeof sqlTutorialData];
    ...
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={handleHomePress} style={styles.homeButton}>
          <MaterialCommunityIcons name="home" size={34} color="#4479A1" />
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  icon: { marginRight: 10 },
  title: { fontSize: 22, fontWeight: 'bold', color: '#4479A1' },
